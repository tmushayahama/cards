...
main() {
  group('Card App Test', () {
    ...
    test('should show a loading screen while the cards are fetched', () async {
      expect(await homeScreen.isLoading(), isTrue);
    });

    test('should start with a list of Cards', () async {
      expect(await homeScreen.isReady(), isTrue);
      expect(await homeScreen.cardList.cardItem('1').isVisible, isTrue);
      expect(await homeScreen.cardList.cardItem('2').isVisible, isTrue);
      expect(await homeScreen.cardList.cardItem('3').isVisible, isTrue);
      expect(await homeScreen.cardList.cardItem('4').isVisible, isTrue);
      expect(await homeScreen.cardList.cardItem('5').isVisible, isTrue);
    });

    test('should be able to click on an item to see details', () async {
      final detailsScreen = await homeScreen.cardList.cardItem('2').tap();
      expect(await detailsScreen.task, isNotEmpty);
      expect(await detailsScreen.note, isNotEmpty);

      final editScreen = detailsScreen.tapEditCardButton();
    ...
    test('should be able to delete a card on the details screen', () async {
      final detailsScreen = new DetailsTestScreen(driver);

      await detailsScreen.tapDeleteButton();

      expect(await homeScreen.cardList.cardItem('2').isAbsent, isTrue,
          reason: 'CardItem2 should be absent');
      expect(await homeScreen.snackbarVisible, isTrue,
          reason: 'snackbar should be visible');
    });

    test('should filter to completed cards', () async {
    ...
    test('should filter to active cards', () async {
    ...
    test('should once again filter to all cards', () async {
    ...
    test('should be able to toggle a card complete', () async {
      await homeScreen.tapCardsTab().cardItem('1').tapCheckbox();
    ...
    test('should be able to clear the completed cards', () async {
    ...
    test('should be able to toggle all cards complete', () async {
    ...
    test('should be able to modify a card', () async {
      ...
      // find the _card text to edit and go to details screen
      final detailsScreen = await homeScreen.tapCard(task);
      expect(await detailsScreen.isReady(), isTrue);

      // go to edit screen and edit this _card
      final editScreen = detailsScreen.tapEditCardButton();
      expect(await editScreen.isReady(), isTrue);
